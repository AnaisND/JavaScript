...
const DisplayHotelsFormSteleC = () => {
  const {clientId, stele} = useParams();
  ...
  useEffect(() => {
    const fetchHotels = async () => {
      try {
        const response = await fetch(`http://localhost:8080/data/ht/stele/${stele}`);
        if (response.ok) {
...
export default DisplayHotelsFormSteleC;
