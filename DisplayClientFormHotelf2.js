...
const DisplayHotelsFormf2C = () => {
  const {stele, clientId} = useParams();
  ...
  useEffect(() => {
    const fetchHotels = async () => {
      try {
        const response = await fetch(`http://localhost:8080/data/ht/f2/${stele}`);
        if (response.ok) {
...
export default DisplayHotelsFormf2C;
